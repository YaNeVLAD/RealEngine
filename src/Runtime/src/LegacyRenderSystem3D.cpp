...
LegacyRenderSystem3D::LegacyRenderSystem3D(render::IWindow& window)
	: m_window(window)
{
}

void LegacyRenderSystem3D::Update(ecs::Scene& scene, const core::TimeDelta dt)
{
	...
	render::Renderer3D::SetEnvironment(skybox.m_irradianceID);
...
void LegacyRenderSystem3D::ProcessTransforms(ecs::Scene& scene)
{
...
void LegacyRenderSystem3D::RebuildStaticOpaqueBatches(ecs::Scene& scene)
{
...
void LegacyRenderSystem3D::CollectDynamicAndTransparent(ecs::Scene& scene, const glm::vec3& camPos)
{
	m_opaqueQueue.clear();
...
void LegacyRenderSystem3D::RenderOpaqueGeometry(const glm::vec3& camPos, float farClip)
{
...
void LegacyRenderSystem3D::RenderTransparentGeometry()
{
...
void LegacyRenderSystem3D::DrawFlatBatch(const std::vector<StaticBatchItem>& batch)
{
	if (batch.empty())
	{
		return;
...
void LegacyRenderSystem3D::ExecuteRenderCommand(const RenderCommand3D& cmd)
{
...
SkyboxComponent LegacyRenderSystem3D::ExtractSkybox(ecs::Scene& scene)
{
	...
	for (auto&& [entity, skybox] : *scene.CreateView<SkyboxComponent>())
	{
		if (skybox.m_hdrTexture && skybox.m_cubemapID == 0)
		{
			...
			skybox.m_irradianceID = render::Renderer3D::CreateIrradianceMap(skybox.m_cubemapID);
		}
