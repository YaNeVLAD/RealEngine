...
inline void EntityManager::DestroyEntity(const Entity entity)
{
	if (!IsValid(entity))
	{
		return;
...
inline bool EntityManager::IsValid(const Entity entity) const
{
	const auto index = entity.Index();
	return index < m_generations.size() && entity.Generation() == m_generations[index];
}

inline void EntityManager::SetSignature(const Entity entity, Signature const& signature)
{
	assert(IsValid(entity) && "Entity is not valid");
	m_signatures[entity.Index()] = signature;
}

inline Signature& EntityManager::GetSignature(const Entity entity)
{
	assert(IsValid(entity) && "Entity is not valid");
	return m_signatures[entity.Index()];
}

inline Signature const& EntityManager::GetSignature(const Entity entity) const
{
